		...
		if(target.m_bAttackBySea)
		{
			...
			if(IsAttackReady(MUFORMATION_NAVAL_INVASION, AI_OPERATION_NAVAL_SNEAK_ATTACK))
			{
				...
				if(target.m_pTargetCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
				{
					...
					if(pOperation != NULL)
					{
						m_pPlayer->GetMilitaryAI()->RequestPureNavalAttack(target.m_pTargetCity->getOwner(), 1);
					}
				}
				...
				if(!target.m_pTargetCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
				{
					...
					if(pBestCity != NULL)
					{
						...
						if(pOperation != NULL)
						{
							...
							{
								CvAIOperation* pNextOperation = 0;
								pNextOperation = m_pPlayer->addAIOperation(AI_OPERATION_PURE_NAVAL_CITY_ATTACK, target.m_pTargetCity->getOwner(), target.m_pTargetCity->getArea(), target.m_pTargetCity, pBestCity);
							}
						}
					}
					if(pOperation != NULL && !pOperation->ShouldAbort())
			...
			if(IsAttackReady(MUFORMATION_NAVAL_INVASION, AI_OPERATION_NAVAL_SNEAK_ATTACK))
			{
				...
				if(pOperation != NULL)
				{
					...
					{
						CvAIOperation* pNextOperation = 0;
						pNextOperation = m_pPlayer->addAIOperation(AI_OPERATION_PURE_NAVAL_CITY_ATTACK, target.m_pTargetCity->getOwner(), target.m_pTargetCity->getArea(), target.m_pTargetCity, target.m_pMusterCity);
					}
				}
#else
			if(IsAttackReady(MUFORMATION_NAVAL_INVASION, AI_OPERATION_SNEAK_CITY_ATTACK))
		...
		if(kTarget.m_bAttackBySea)
		{
			...
			if(kTarget.m_pTargetCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
			{
				...
				if((iNumRequiredSlots - iFilledSlots) <= iNumUnitsWillingToBuild && iLandReservesUsed <= GetLandReservesAvailable())
				{
					...
					if(pOperation != NULL)
					{
						m_pPlayer->GetMilitaryAI()->RequestPureNavalAttack(kTarget.m_pTargetCity->getOwner(), 1);
					}
				}
			}
			//Not coastal? Let's make sure we have the best muster target for this, and that we send the right kind of force.
				...
				if(pBestCity != NULL)
				{
					...
					if((iNumRequiredSlots - iFilledSlots) <= iNumUnitsWillingToBuild && iLandReservesUsed <= GetLandReservesAvailable())
					{
						...
						if(pOperation != NULL)
						{
							...
							{
								CvAIOperation* pNextOperation = 0;
								pNextOperation = m_pPlayer->addAIOperation(AI_OPERATION_PURE_NAVAL_CITY_ATTACK, kTarget.m_pTargetCity->getOwner(), kTarget.m_pTargetCity->getArea(), kTarget.m_pTargetCity, pBestCity);
							}
						}
					}
				}
				...
				else
				{
					...
					if((iNumRequiredSlots - iFilledSlots) <= iNumUnitsWillingToBuild && iLandReservesUsed <= GetLandReservesAvailable())
					{
						...
						if(pOperation != NULL)
						{
							...
							{
								CvAIOperation* pNextOperation = 0;
								pNextOperation = m_pPlayer->addAIOperation(AI_OPERATION_PURE_NAVAL_CITY_ATTACK, kTarget.m_pTargetCity->getOwner(), kTarget.m_pTargetCity->getArea(), kTarget.m_pTargetCity, kTarget.m_pMusterCity);
							}
						}
					}
				}
...
void CvMilitaryAI::ShouldAttackBySea(PlayerTypes eEnemy, CvMilitaryTarget& target)
{
	...
	if(GET_TEAM(m_pPlayer->getTeam()).canEmbark())
	{
...
#if defined(MOD_BALANCE_CORE_MILITARY)
		// Is the route too difficult? We're looking for lots of mountains here. It is infinitely easier to travel by sea if our formation will break down because of narrow paths.
		if(MOD_BALANCE_CORE_MILITARY)
		{
			...
			if(GC.getStepFinder().GeneratePath(target.m_pMusterCity->getX(), target.m_pMusterCity->getY(), target.m_pTargetCity->getX(), target.m_pTargetCity->getY()))
			{
